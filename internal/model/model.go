...
// down to the selected leafs, as well as all leafs that are not tranversed but
...
func (m *Model) GetSelections(folder string, tree bool) (map[string]interface{}, error) {
	...
	if tree {
		// Builds a jsTree plugin parsable tree which indicates leafs and their
		// either their children ([]*jsTreeLeaf) or presence of children (bool)
		...
		files.WithGlobalTruncated(func(fi db.FileIntf) bool {
			...
			if parent == "." {
				...
				if leaf, ok := leafs[parent]; ok && !leaf.Edge {
					parentMatch = true
					grandparentMatch = true
				} else if leaf, ok := leafs[grandparent]; ok && !leaf.Edge {
					grandparentMatch = true
				}
			...
			if !grandparentMatch {
				...
				// our parent is not one of the edge leafs who would needs to
			...
			if selfMatch {
				...
				self.Children = make([]*jsTreeLeaf, 0)
			} else if parentMatch {
				self.Edge = true
				self.Children = false
			}
			...
			children := par.Children.([]*jsTreeLeaf)
			// If we are the first child, add the "Files in XYZ" leaf
			// before us.
			if len(children) == 0 {
				children = append(children, &jsTreeLeaf{
